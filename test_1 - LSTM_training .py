...
def forecast_pipeline(df,
                      ...
                      n_past: int = 10,       # lookback window used by create_sequences
                      units: int = 64,        # LSTM hidden size
                      depth: int = 2,         # number of stacked LSTM layers
                      dropout: float = 0.2,   # dropout + recurrent_dropout
    ...
    for i in range(depth):
        x = layers.LSTM(
            units,
            return_sequences=(i < depth - 1),      # keep sequences for all but last layer
            dropout=dropout,
        ...
        x = layers.LayerNormalization()(x)          # helps stabilize training on mixed-scale features
    ...
    model.compile(
        optimizer=optimizer,
        loss=keras.losses.Huber(delta=1.0),        # better than MSE for outliers
        metrics=[
